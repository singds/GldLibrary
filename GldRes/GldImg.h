...
typedef struct
{
    ...
    /* corrisponde in valore assoluto all'altezza dell'immagine in pixel:
        quando il valore � positivo
            l'immagine � bottom-up (la mappa dei pixel incomincia dalla riga di
            pixel pi� in basso e finisce con quella pi� in alto). Questa � la
            variante pi� comune.
        quando il valore � negativo
            l'immagine � top-down (la mappa dei pixel incomincia dalla riga di
            pixel pi� in alto e finisce con quella pi� in basso). */
    uint16_t BiPlanes;
    /* sempre 1 */
    uint16_t BiBitCount;
    /* profondit� di colore dell'immagine in bit per pixel, dev'essere uno dei
        ...
        versione 5 del formato si pu� usare il valore 0 quando viene incapsulata
        un'immagine JPEG o PNG. */
    uint32_t BiCompression;
    /* uno dei seguenti valori:
        0 (BI_RGB)
            La mappa dei pixel non � compressa.
        1 (BI_RLE8)
            La mappa dei pixel � compressa con l'algoritmo RLE per 8 bit per pixel.
            Valido solo per biBitCount = 8 e biHeight > 0.
        2 (BI_RLE4)
            La mappa dei pixel � compressa con l'algoritmo RLE per 4 bit per pixel.
            Valido solo per biBitCount = 4 e biHeight > 0.
        3 (BI_BITFIELDS)
            La mappa dei pixel non � compressa ed � codificata secondo maschere di
    ...
    /* Indica la dimensione in byte del buffer mappa dei pixel. Questo valore
        pu� essere lasciato a zero quando biCompression � impostato a BI_RGB. */
    uint32_t BiXPelsPerMeter;
    /* risoluzione orizzontale del dispositivo di output in pixel per metro;
        0 se la risoluzione non � specificata. */
    ...
    /* quando biBitCount = 1
        ...
        altrimenti
            ...
            Per profondit� maggiori di 8 bit per pixel la tavolozza non � normalmente
            necessaria, ma quando c'� pu� essere usata dal sistema o da alcuni programmi
